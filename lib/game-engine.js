...
class GameEngine {
  ...
  async attemptAnswer (rawGuess, msg) {
    ...
    if (AnswerComparison.compare(rawGuess, this.currentQuestion.answer)) {
      ...
      await Scoreboard.decreaseScore(msg.author, this.currentQuestion.value / 2);
      const newScore = await Scoreboard.getScore(msg.author);
      msg.channel.send(this._formatIncorrectAnswer({ user: msg.author, newScore }));
    }
  }
  ...
  _formatIncorrectAnswer ({ user, newScore }) {
    return `That is incorrect, ${user}. Your score is now $${newScore}.`;
  }
}
